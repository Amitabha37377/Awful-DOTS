...
local create_button = function(icon, launch_directory, mleft, mright, mtop, mbottom)
  ...
  button:connect_signal("button::press", function(_, _, _, button)
    if button == 1 then
      awful.spawn.with_shell(launch_directory)
    end
  end)
...
local directories = {
  home = create_button('user-home.svg', 'thunar', 3, 0, 0, 1),
  documents = create_button('folder-documents.svg', 'thunar ~/Documents/', 0, 0, 0, 2),
  downloads = create_button('folder-downloads.svg', 'thunar ~/Downloads/', 0, 0, 0, 2),
  config = create_button('folder-development.svg', 'thunar ~/.config', 0, 0, 0, 2),
}
