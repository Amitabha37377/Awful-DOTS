			...
			{
				{
					widget = wibox.widget.textbox,
					markup = '<span color="' ..
							color.lightblue .. '" font="Ubuntu Nerd Font bold 12">' .. pickedcolor .. '</span>',
...
local window = awful.popup {
	...
	placement = function(c)
		awful.placement.bottom_right(c,
			{ margins = { top = dpi(0), bottom = dpi(85), left = dpi(0), right = dpi(400) } })
	end,
...
-------------------------------
awesome.connect_signal("picker::show", function(_, _, _, button)
	window.visible = not window.visible
	if window.visible then
		awesome.emit_signal("dock::pausehide")
	else
		awesome.emit_signal("dock::hideagain")
	end
end)


awesome.connect_signal("picker::pick", function(_, _, _, button)
