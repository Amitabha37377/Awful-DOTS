...
local create_container = function(img, clr, text, onstatus, fn_on, fn_off)
	...
	txt.halign = 'center'
	txt.foced_width = dpi(60)

	local container =
			wibox.widget {
				helpers.margin(btn, 0, 0, 0, 4),
				txt,
	...
	return helpers.margin(container, 20, 20, 10, 15)
end
...
local containers = {
	...
	night_mode = create_container("dnd.png", color.blue, "Night", false,
		function() awful.spawn.with_shell('redshift -l 0:0 -t 4500:4500 -r') end,
		function() awful.spawn.with_shell("redshift -x && killall redshift") end
	),
